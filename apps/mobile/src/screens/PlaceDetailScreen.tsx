...
export default function PlaceDetailScreen({ place, source }: { place: DiscoveryPlace; source: DiscoveryDataSource }) {
  ...
  useEffect(() => {
    let cancelled = false;
    void Promise.all(place.people.map((id) => source.getPerson(id))).then((results) => {
      if (!cancelled) setPeople(results.filter((person): person is DiscoveryPerson => Boolean(person)));
    });
    void Promise.all(place.relatedPlaces.map((id) => source.getPlace(id))).then((results) => {
      if (!cancelled) setRelatedPlaces(results.filter((related): related is DiscoveryPlace => Boolean(related)));
    });
    return () => { cancelled = true; };
  }, [place.people, place.relatedPlaces, source]);
  ...
  return (
    <ScrollView style={{ backgroundColor: theme.colors.background }} contentContainerStyle={detailStyles.content}>
      ...
      <View style={[detailStyles.hero, { backgroundColor: category.colour }]}><View style={detailStyles.heroTexture} /><View style={detailStyles.heroIcon}><IconGlyph symbol={category.symbol} colour={theme.colors.white} size={32} /></View><Text style={detailStyles.heroCategory}>{category.label.toUpperCase()}  ·  {place.placeType.replace(/_/g, ' ')}</Text><Text style={[detailStyles.heroTitle, { fontFamily: theme.typography.editorial }]}>{place.name}</Text><Text style={detailStyles.heroLocation}>{place.location.label}</Text></View>
      <View style={detailStyles.actionRow}><SaveButton saved={saved} onPress={() => toggleSaved(place.id, place.saved)} /><Pressable accessibilityRole="button" accessibilityState={{ selected: visited }} onPress={() => toggleVisited(place.id, place.visited)} style={[detailStyles.actionButton, { backgroundColor: visited ? theme.colors.accentSoft : theme.colors.surface, borderColor: visited ? theme.colors.accent : theme.colors.border }]}><Text style={[detailStyles.actionText, { color: visited ? theme.colors.accentStrong : theme.colors.text }]}>{visited ? 'Visited ✓' : 'Mark visited'}</Text></Pressable><Pressable accessibilityRole="link" onPress={() => undefined} style={[detailStyles.actionButton, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}><Text style={[detailStyles.actionText, { color: theme.colors.text }]}>Directions ↗</Text></Pressable></View>
...
const detailStyles = StyleSheet.create({
  ...
  heroIcon: { width: 58, height: 58, borderRadius: 8, backgroundColor: '#00000025', alignItems: 'center', justifyContent: 'center', marginBottom: 38 },
  heroCategory: { color: '#ffffffd9', fontSize: 10, fontWeight: '900', letterSpacing: 0.9 },
  heroTitle: { color: '#fff', fontSize: 28, lineHeight: 32, fontWeight: '900', letterSpacing: -0.7, marginTop: 5 },
  heroLocation: { color: '#ffffffd9', fontSize: 13, marginTop: 5 },
  actionRow: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 16 },
  actionButton: { minHeight: 38, borderWidth: 1, borderRadius: 4, paddingHorizontal: 11, justifyContent: 'center' },
