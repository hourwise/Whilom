...
-- Two tight knots plus one outlier, so aggregation has something to do.
--
-- Deliberately placed over south Wales rather than Yorkshire: the repository
-- seed inserts five real Yorkshire places, and they were landing inside the
-- test viewport and quietly adding cells to every count below.
insert into public.places (slug, name, place_type, location, status, content_level)
select
  'cluster-a-' || i, 'Cluster A ' || i, 'building',
  extensions.st_setsrid(extensions.st_makepoint(-3.500 + i * 0.0001, 51.500 + i * 0.0001), 4326)::extensions.geography,
  'approved', 1
...
select
  ...
  extensions.st_setsrid(extensions.st_makepoint(-3.200 + i * 0.0001, 51.700 + i * 0.0001), 4326)::extensions.geography,
  'approved', 1
from generate_series(1, 25) i;

insert into public.places (slug, name, place_type, location, status, content_level)
values ('lonely', 'Lonely Barn', 'building',
  extensions.st_setsrid(extensions.st_makepoint(-3.900, 51.900), 4326)::extensions.geography, 'approved', 1);
...
values ('bbbbbbbb-0000-0000-0000-000000000001', 'draft-hall', 'Draft Hall', 'country_house',
  extensions.st_setsrid(extensions.st_makepoint(-3.4995, 51.5005), 4326)::extensions.geography, 'needs_review');

-- ---------------------------------------------------------------------------
-- Aggregation
-- ---------------------------------------------------------------------------
select is(
  (select count(*) from public.map_clusters(-4.0, 51.3, -3.0, 52.0, 0.1)),
  3::bigint, 'a broad view returns one cell per knot of places, not one row per place');

select is(
  (select sum(place_count)::bigint from public.map_clusters(-4.0, 51.3, -3.0, 52.0, 0.1)),
  66::bigint, 'and the cell counts add up to every published place in view');

select ok(
  (select max(place_count) from public.map_clusters(-4.0, 51.3, -3.0, 52.0, 0.1)) = 40,
...
select is(
  (select sum(place_count)::bigint from public.map_clusters(-3.51, 51.49, -3.49, 51.51, 0.1)),
...
select is(
  (select count(*) from public.map_clusters(-4.0, 51.3, -3.0, 52.0, 0.1, array['castle'])),
...
select ok(
  (select count(*) from public.map_clusters(-4.0, 51.3, -3.0, 52.0, 0.0000001))
    <= 2000,
  'an absurdly fine grid is clamped rather than honoured');

select ok(
  (select count(*) from public.map_clusters(-4.0, 51.3, -3.0, 52.0, 0.1, null, null, null, null, null, null, false, 2))
    <= 2,
...
select ok(
  (select count(*) from public.map_places(-3.51, 51.49, -3.49, 51.51, null, 500)) > 0,
...
select throws_ok(
  $$insert into public.places (slug, name, place_type, location)
     values ('sneaky', 'Sneaky', 'building',
       extensions.st_setsrid(extensions.st_makepoint(-3.5, 51.5), 4326)::extensions.geography)$$,
